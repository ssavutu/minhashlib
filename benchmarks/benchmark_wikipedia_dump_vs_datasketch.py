...
def normalize_wiki_text(text: str) -> str:
    # Keep cleanup lightweight and deterministic for benchmarking.
    text = re.sub(r"\{\{[^{}]*\}\}", " ", text)
...
def load_docs(cfg: BenchmarkConfig) -> list[str]:
    docs: list[str] = []
    for raw_text in iter_wikipedia_texts(cfg.dump_path):
        cleaned = normalize_wiki_text(raw_text)
        if len(cleaned) >= max(cfg.k, cfg.min_chars):
            docs.append(cleaned)
...
def parse_args() -> BenchmarkConfig:
    parser = argparse.ArgumentParser(
        description="Benchmark DiffChecker vs datasketch.MinHash on a Wikipedia XML dump."
    )
    parser.add_argument("--dump-path", type=Path, required=True, help="Path to .xml/.xml.bz2/.xml.gz dump")
...
def main():
    cfg = parse_args()
    if not cfg.dump_path.exists():
        raise SystemExit(f"Dump path not found: {cfg.dump_path}")

    base_docs = load_docs(cfg)
    if len(base_docs) < 2:
        raise SystemExit("Not enough documents extracted from dump. Lower --min-chars or increase dump size.")

    random_pairs = make_random_pairs(cfg, len(base_docs))
    near_dup_docs, near_dup_pairs = make_near_duplicates(cfg, base_docs)

    print("Benchmark: DiffChecker vs datasketch.MinHash on Wikipedia dump")
    print(
        "Config:",
        f"dump_path={cfg.dump_path}, max_docs={cfg.max_docs}, k={cfg.k}, num_perm={cfg.num_perm},",
        f"random_pairs={cfg.random_pairs}, near_dup_pairs={cfg.near_dup_pairs}, min_chars={cfg.min_chars}, seed={cfg.seed}",
    )
    print()
